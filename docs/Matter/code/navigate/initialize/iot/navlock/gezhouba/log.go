...
func createWriteSyncer(filename string) zapcore.WriteSyncer {
	if len(filename) == 0 {
		filename = cm.PathJoin(glogconf.Director, glogconf.Rotated.Filename)
...
func InitLogger() *zap.Logger {
	glogconf.Director = cm.DealWithExecutingCurrentFilePath(glogconf.Director)
	// 判断是否有Director文件夹
	if ok, err := cm.PathExists(glogconf.Director); !ok {
...
func Zap() (logger *zap.Logger) {
	...
	cores := [...]zapcore.Core{
		...
		getEncoderCore(cm.PathJoin(glogconf.Director, "debug.log"), debugPriority),
		getEncoderCore(cm.PathJoin(glogconf.Director, "info.log"), infoPriority),
		getEncoderCore(cm.PathJoin(glogconf.Director, "warn.log"), warnPriority),
		getEncoderCore(cm.PathJoin(glogconf.Director, "error.log"), errorPriority),
	}
