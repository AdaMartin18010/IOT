...
impl JsonSchemaValidator {
    ...
    fn load_predefined_schemas(&mut self) {
        ...
        // 追加：在 JsonSchemaValidator::load_predefined_schemas 中注册新增标准 Schema
        let lorawan_schema = serde_json::json!({
          "type": "object",
          "properties": {
            "network": {"type": "object", "properties": {"frequency_plan": {"type": "string"}, "duty_cycle": {"type": "number", "minimum": 0.0, "maximum": 1.0}}},
            "device": {"type": "object", "properties": {"activation": {"type": "string", "enum": ["OTAA","ABP"]}, "dr": {"type": "integer", "minimum": 0, "maximum": 15}}},
            "security": {"type": "object", "properties": {"nwk_key": {"type": "string"}, "app_key": {"type": "string"}}}
          },
          "required": ["network","device"]
        });
        if let Ok(schema) = JSONSchema::compile(&lorawan_schema) { self.schemas.insert("lorawan".to_string(), schema); }

        let zigbee_schema = serde_json::json!({
          "type": "object",
          "properties": {
            "network": {"type": "object", "properties": {"channel": {"type": "integer", "minimum": 11, "maximum": 26}, "pan_id": {"type": "string"}}},
            "topology": {"type": "string", "enum": ["star","tree","mesh"]},
            "security": {"type": "object", "properties": {"link_key": {"type": "string"}}}
          },
          "required": ["network","topology"]
        });
        if let Ok(schema) = JSONSchema::compile(&zigbee_schema) { self.schemas.insert("zigbee".to_string(), schema); }

        let thread_schema = serde_json::json!({
          "type": "object",
          "properties": {
            "ipv6": {"type": "object", "properties": {"prefix": {"type": "string"}}},
            "roles": {"type": "array", "items": {"type": "string", "enum": ["leader","router","child","border_router"]}},
            "security": {"type": "object", "properties": {"network_key": {"type": "string"}}}
          },
          "required": ["ipv6","roles"]
        });
        if let Ok(schema) = JSONSchema::compile(&thread_schema) { self.schemas.insert("thread".to_string(), schema); }

        let nbiot_schema = serde_json::json!({
          "type": "object",
          "properties": {
            "cellular": {"type": "object", "properties": {"plmn": {"type": "string"}, "apn": {"type": "string"}}},
            "qos": {"type": "object", "properties": {"latency_ms": {"type": "integer", "minimum": 1}, "reliability": {"type": "number", "minimum": 0.0, "maximum": 1.0}}}
          },
          "required": ["cellular"]
        });
        if let Ok(schema) = JSONSchema::compile(&nbiot_schema) { self.schemas.insert("nbiot".to_string(), schema); }
    }
            ...
            "production" => {
                ...
                if entry.environment == "production" {
                  match entry.namespace.as_str() {
                    "lorawan" => {
                      if let Some(net) = entry.value.get("network") {
                        if let Some(freq) = net.get("frequency_plan").and_then(|v| v.as_str()) {
                          if freq.contains("test") { errors.push("LoRaWAN 生产环境禁止使用测试频段".into()); }
                        }
                      }
                    }
                    "zigbee" => { /* 校验信道、PAN ID 合规性 */ }
                    "thread" => { /* 校验前缀与路由角色配置 */ }
                    "nbiot" => { /* 校验APN/PLMN 合规 */ }
                    _ => {}
                  }
...
## 6. 与最新扩展计划对齐（新增标准与互操作配置）

### 6.1 新增标准命名空间与Schema

- 新增命名空间：`lorawan`、`zigbee`、`thread`、`nbiot`
- 每个命名空间需注册对应 JSON Schema，纳入 `JsonSchemaValidator` 管理。
...
- 生产环境强校验：禁止使用测试频段/测试密钥；`production` 环境触发更严格规则。
- 开发/测试环境放宽并提示警告：允许 `localhost`、低强度密钥，仅作为演示用途。

```rust
// 在 validate 环节增加命名空间与环境的联动校验
if entry.environment == "production" {
  match entry.namespace.as_str() {
    "lorawan" => {
      if let Some(net) = entry.value.get("network") {
...
```yaml
# config/interoperability/cross-standard.yml
interoperability:
  bridges:
    - name: lorawan-opcua
      source: lorawan
      target: opcua
      mappings:
        - from: device.uplink.payload
          to: opcua.nodes.sensors.temperature.value
          transform: "base64_decode|json_path:$.temp"
    - name: zigbee-matter
      source: zigbee
      target: matter
      mappings:
        - from: cluster.onoff.state
          to: matter.cluster.onoff
          transform: "identity"
    - name: thread-5giot
      source: thread
      target: 5g-iot
      mappings:
        - from: ipv6.prefix
          to: 5g.slice.qos.routing_prefix
          transform: "identity"
```

### 6.4 灰度与版本策略（配置中心驱动）

- 使用 `tags` 与 `namespace` 组合进行灰度：按设备标签/区域分配不同配置版本。
- 通过 `ConfigVersion` 历史记录支持快速回滚与差异比对。

```yaml
# ConfigMap（Kubernetes）示例：分区域灰度
data:
  rollout.yaml: |
    rollout:
      namespace: lorawan
      ...
      segments:
        ...
        - selector: { region: "ap-south" }
          percentage: 10
      successCriteria:
        - metric: lorawan.join.accept.rate
          threshold: ">=0.995"
        - metric: uplink.latency.p95
          threshold: "<=200ms"
```

### 6.5 API 使用示例（创建/更新新增标准配置）

```bash
# 创建 LoRaWAN 配置（development 环境）
curl -X POST http://config-api/api/v1/config -H 'Content-Type: application/json' -d @- <<'JSON'
{
  "key": "lorawan-network",
  "value": {"network": {"frequency_plan":"EU868-test","duty_cycle":0.01},"device":{"activation":"OTAA","dr":5}},
  "environment": "development",
  "namespace": "lorawan",
  "tags": {"region":"eu-west","segment":"canary"}
}
JSON

# 生产环境更新将触发严格校验
curl -X PUT http://config-api/api/v1/config/production/lorawan-network -H 'Content-Type: application/json' -d @- <<'JSON'
{ "value": {"network": {"frequency_plan":"EU868","duty_cycle":0.01},"device":{"activation":"OTAA","dr":5}} }
JSON
```

以上改动确保配置管理中心对齐“新增标准、互操作桥接、环境化/灰度策略”的最新计划，并与CI/CD、互操作测试矩阵保持一致。
